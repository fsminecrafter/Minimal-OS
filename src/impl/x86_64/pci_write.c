...
// I/O port read/write (use inline asm or compiler intrinsics)
// Replace with your platform-specific implementation if needed.
...
// Write 8-bit to PCI config space
...
// Read 64-bit from PCI config space (as two 32-bit reads)
uint64_t pci_config_read64(uint8_t bus, uint8_t device, uint8_t function, uint8_t offset) {
...
// Write 64-bit to PCI config space (as two 32-bit writes)
void pci_config_write64(uint8_t bus, uint8_t device, uint8_t function, uint8_t offset, uint64_t value) {
