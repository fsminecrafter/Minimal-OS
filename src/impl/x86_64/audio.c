...
static const uint8_t* audio_find_bytes(const uint8_t* haystack, uint32_t haystack_len,
    ...
    return NULL;
                                        ...
                                        * ring slot, and returns the number of PCM samples written.
                                        *
                                        * Chunk-size contract (CRITICAL):
                                        *   pcm_capacity = AUDIO_BUFFER_SIZE * channels  (samples per slot)
                                        *
                                        ...
                                        * ============================================================ */
                                       static uint32_t audio_player_fill_buffer(audio_player_t* player, uint8_t slot) {
                                           ...
                                           (uint32_t)slot * player->pcm_capacity;
                                        ...
                                        * Drains decoded PCM from the player's ring buffer.
                                        * ============================================================ */
                                       void audio_mix_streams(int16_t* out, uint32_t frames) {
                                           ...
                                           while (out_idx < out_samples) {
                                               /* ---- Current slot exhausted (or never had data)? ---- */
                                               if (!player->ring_ready[player->read_slot] ||
                                                   player->read_pos >= player->ring_size[player->read_slot]) {

                                                   /* Free the slot we just finished draining so the producer
                                                    * can reuse it. Harmless no-op if it was never filled. */
                                                   player->ring_ready[player->read_slot] = false;
                                               player->ring_size[player->read_slot]  = 0;

                                               uint8_t next_slot = (uint8_t)((player->read_slot + 1) % AUDIO_RING_SIZE);

                                               if (!player->ring_ready[next_slot]) {
                                                   /* Background task hasn't decoded the next slot yet -
                                                    * genuine underrun. Fill the remainder of this
                                                    * hardware buffer with silence and leave read_slot
                                                    * where it is so we check the same next_slot again
                                                    * next call, once it's ready. */
                                               ...
                                               player->read_slot = next_slot;
                                               player->read_pos  = 0;
                                                   }

                                                   int16_t* slot_buf = player->pcm_buffer +
                                                   (uint32_t)player->read_slot * player->pcm_capacity;

                                                   if (src_channels <= 1) {
                                                       /* Mono → duplicate to all output channels */
                                                       int16_t s = audio_apply_volume(
                                                           slot_buf[player->read_pos++], volume);
                                                       if (out_channels == 1) {
                                                           out[out_idx++] = s;
                                                       } else {
                                                           out[out_idx++] = s;
                                                       ...
                                                       if (player->read_pos + 2 > player->ring_size[player->read_slot]) {
                                                           /* Odd leftover sample - force the exhausted check at
                                                            * the top of the loop to run again next iteration. */
                                                           player->read_pos = player->ring_size[player->read_slot];
                                                           continue;
                                                       }
                                                       ...
                                                       if (src_channels > 2) {
                                                           ...
                                                           if (player->read_pos + skip <= player->ring_size[player->read_slot])
                                                               player->read_pos += skip;
                                                           else
                                                               player->read_pos = player->ring_size[player->read_slot];
                                                       }
                                                                                           ...
                                                                                           bool decode_flac(const uint8_t* input, uint32_t input_size,
                                                                                               ...
                                                                                               return false;
                                                                                                            ...
                                                                                                            audio_player_t* audio_player_create(const char* path) {
                                                                                                                 ...
                                                                                                                 * We want to fill exactly ONE ring slot (pcm_capacity samples) per
                                                                                                                 * readstream() call.
                                                                                                                 *
                                                                                                                ...
                                                                                                                /*
                                                                                                                 * pcm_capacity = samples in ONE ring slot.
                                                                                                                ...
                                                                                                                serial_write_dec(AUDIO_RING_SIZE);
                                                                                                                serial_write_str("\n");
                                                                                                                ...
                                                                                                                /* Ring buffer: AUDIO_RING_SIZE slots of pcm_capacity samples each */
                                                                                                                uint32_t total_samples = pcm_capacity * AUDIO_RING_SIZE;
                                                                                                                ...
                                                                                                                for (int i = 0; i < AUDIO_RING_SIZE; i++) {
                                                                                                                    player->ring_ready[i] = false;
                                                                                                                    player->ring_size[i]  = 0;
                                                                                                                }
                                                                                                            ...
                                                                                                            void audio_player_play(audio_player_t* player) {
                                                                                                                ...
                                                                                                                /*
                                                                                                                 * Reset ring state and prefill every slot up front. This gives the
                                                                                                                 * mixer/hardware the full AUDIO_RING_SIZE worth of lead time right
                                                                                                                 * from the first tick, matching AC97's own fill-ahead target
                                                                                                                 * instead of starting with only two buffers ready and hoping the
                                                                                                                 * background task catches up before hardware runs dry.
                                                                                                                 */
                                                                                                                player->read_slot  = 0;
                                                                                                                player->read_pos   = 0;
                                                                                                                player->write_slot = 0;
                                                                                                                ...
                                                                                                                serial_write_str("[PLAYER] Prefilling ring buffers...\n");

                                                                                                                uint8_t filled = 0;
                                                                                                                for (int i = 0; i < AUDIO_RING_SIZE; i++) {
                                                                                                                    uint32_t samples = audio_player_fill_buffer(player, (uint8_t)i);
                                                                                                                    if (samples == 0) break;   // short file / EOF - stop prefilling
                                                                                                                    player->ring_size[i]  = samples;
                                                                                                                    player->ring_ready[i] = true;
                                                                                                                    player->write_slot    = (uint8_t)((i + 1) % AUDIO_RING_SIZE);
                                                                                                                    filled++;
                                                                                                                }
                                                                                                                ...
                                                                                                                serial_write_str("[PLAYER] Ring prefilled, slots=");
                                                                                                                serial_write_dec(filled);
                                                                                                                serial_write_str("\n");
                                                                                                            ...
                                                                                                            bool audio_player_update(audio_player_t* player) {
                                                                                                                ...
                                                                                                                /*
                                                                                                                 * Fill every ring slot that isn't ready yet. Unlike the old
                                                                                                                 * single-shot "needs_refill" handshake (which only ever refilled
                                                                                                                 * one buffer per call, even if playback had fallen behind by
                                                                                                                 * more), this lets one invocation catch up on several slots at
                                                                                                                 * once - e.g. after a slow disk read, or after losing a few
                                                                                                                 * scheduling slots to other cooperatively-scheduled processes.
                                                                                                                 */
                                                                                                                bool stream_ended = false;

                                                                                                                for (int i = 0; i < AUDIO_RING_SIZE; i++) {
                                                                                                                    uint8_t slot = player->write_slot;

                                                                                                                    if (player->ring_ready[slot]) {
                                                                                                                        /* Already full - move on in case a later slot is free. */
                                                                                                                        player->write_slot = (uint8_t)((slot + 1) % AUDIO_RING_SIZE);
                                                                                                                        continue;
                                                                                                                    }

                                                                                                                    uint32_t samples = audio_player_fill_buffer(player, slot);

                                                                                                                    if (samples == 0) {
                                                                                                                        /* Genuine end of stream: audio_player_fill_buffer() already
                                                                                                                         * tried to loop internally if player->loop was set, so a
                                                                                                                         * 0 here means there is truly nothing more to give it. */
                                                                                                                        stream_ended = true;
                                                                                                                        break;
                                                                                                                    }

                                                                                                                    player->ring_size[slot]  = samples;
                                                                                                                    player->ring_ready[slot] = true;
                                                                                                                    player->write_slot       = (uint8_t)((slot + 1) % AUDIO_RING_SIZE);
                                                                                                                }
