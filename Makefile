...
# Auto-detect GitHub Actions toolchain if present
ifeq ($(wildcard $(HOME)/cross/bin/x86_64-elf-gcc),)
else
...
build-x86_64: check-tools $(kernel_object_files) $(x86_64_object_files) $(audio_object_files)
	@mkdir -p dist/x86_64
	$(LD) $(LDFLAGS) -o dist/x86_64/kernel.bin -T targets/x86_64/linker.ld \
...
run-sdl: build-x86_64
	...
	-usb -device usb-kbd \
	-audiodev sdl,id=audio0 -device AC97,audiodev=audio0
...
run-alsa: build-x86_64
	...
	-usb -device usb-kbd \
	-audiodev alsa,id=audio0 -device AC97,audiodev=audio0
